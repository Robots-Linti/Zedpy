...
class OpcionRobot():
    ...
    def __init__(self, config, pilas):
        ...
        ## OPCIONES     ##########################
        ##########################################
        self.txt_id = self.pilas.actores.Texto('', magnitud=30, x=-500, y=-400, fuente=self.fuente)
        self.txt_conexiones = self.pilas.actores.Texto('', magnitud=28, x=130, y=150, fuente=self.fuente)
        self.txt_conexiones.color = self.pilas.colores.amarillo
        self.txt_conexion = self.pilas.actores.Texto('Sin conexion', magnitud=30, x=360, y=-450, fuente=self.fuente)
        self.txt_conexion.color = self.pilas.colores.rojo
        self._board()
        self._nombre()
        self._time()
        self._speed()
        self._id()
        self.btn_robot_refresh=self.pilas.actores.Actor(imagen=self.boton_off,x=-424,y=-300)
      ...
      #  self.btn_robot_refresh.cuando_hace_click=self.robot
        ...
        ########OPCIONES ########################
        ##########################################
        self.pilas.tareas.agregar(1, self._conexion)
    ...
    def conectar(self):
        ...
        try:
            ...
            self.txt_conexion.color = self.pilas.colores.verde
        except:
            self.presence = False
            lista_ids = []
            ...
            if self.presence == True:
                ...
                self.txt_id.color = self.pilas.colores.rojo
        self.btn_robot_refresh.imagen=self.boton_on

    def _id(self):
        self.txtid = self.pilas.actores.Texto('Id del Robot a usar:', x=-380, y=500, fuente=self.fuente, magnitud=30)
        self.id = self.pilas.interfaz.IngresoDeTexto(str(self.config.idrobot), x=-400, y=440, ancho=100)
        self.id.escala = 2
        self.id.solo_numeros()

    def _nombre(self):
        self.txtnombre = self.pilas.actores.Texto('Nombre del Robot:', x=-380, y=360, fuente=self.fuente, magnitud=30)
        self.nombre = self.pilas.interfaz.IngresoDeTexto(self.config.namerobot, x=-350, y=300, ancho=150)
        self.nombre.escala = 2

    def _board(self):
        self.txtboard = self.pilas.actores.Texto('Dispositivo de Comunicacion:', x=-310, y=210, fuente=self.fuente,
                                            magnitud=30)
        self.device = self.pilas.interfaz.IngresoDeTexto(self.config.board, x=-200, y=140)
        self.device.escala = 2

    def _speed(self):
        self.txtspeed = self.pilas.actores.Texto('Speed por Default:', x=-380, y=50, fuente=self.fuente, magnitud=30)
        self.speed = self.pilas.interfaz.IngresoDeTexto(str(self.config.speedrobot), ancho=100, x=-400, y=-20)
        self.speed.escala = 2
        self.speed.solo_numeros()

    def _time(self):
        self.txttime = self.pilas.actores.Texto('Tiempo por Default:', x=-380, y=-100, fuente=self.fuente, magnitud=30)
        self.time = self.pilas.interfaz.IngresoDeTexto(str(self.config.timerobot), ancho=100, x=-400, y=-180)
        self.time.escala = 2
